...
const StockDetail = ({ stock, initialPrices, symbol }: StockDetailProps) => {
  ...
  useEffect(() => {
    if (timeRange !== '1M') fetchPriceData();
  }, [timeRange]);
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  try {
    ...
    if (stock && stock.symbol) {
      initialPrices = await getStockPrices(stock.symbol, 30, 'DAILY');
    }

    return { props: { stock, initialPrices, symbol } };
  } catch (error) {
    console.error(error);
    return { props: { stock: null, initialPrices: [], symbol } };
  }
};
