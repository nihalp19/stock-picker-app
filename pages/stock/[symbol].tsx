...
const StockDetail = ({ stock, initialPrices, symbol }: StockDetailProps) => {
    ...
    useEffect(() => {
        if (timeRange !== '1M' && symbol) fetchPriceData();
    }, [timeRange]);
    ...
    const fetchPriceData = async () => {
        ...
        try {
            ...
            switch (timeRange) {
                case '1D': days = 1; type = 'INTRADAY'; break;
    ...
    return (
        <>
            <Head>
                ...
                <meta name="description" content={`Detailed info and price chart for ${stock.company} (${stock.symbol})`} />
            </Head>
            ...
            <div className="min-h-screen bg-gray-50">
                <div className="container mx-auto px-4 py-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 md:space-x-6">
                        ...
                        <div className="flex items-center space-x-3">
                            <button
                                ...
                                className={`p-2 rounded-full transition-colors ${isFavorite ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                                aria-label={isFavorite ? 'Remove from favorites' : 'Add to favorites'}
                            >
                                <Heart className={`h-5 w-5 ${isFavorite ? 'fill-current' : ''}`} />
                            </button>
                            <button className="p-2 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors" aria-label="Share stock">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6">
                            ...
                            <div className="flex space-x-2 flex-wrap">
                                {(['1D', '1W', '1M', '3M'] as const).map(range => (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-md text-sm ${timeRange === range ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                                    >
                                        {range}
                                    </button>
                                ))}
                        ...
                        {stock.symbol && (
                            <StockGraph data={prices} symbol={stock.symbol} isLoading={isLoading && timeRange !== '1M'} />
                        )}
                    </div>
                </div>
            </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    try {
        ...
        if (safeSymbol) initialPrices = await getStockPrices(safeSymbol, 30, 'DAILY');

        return {
            props: {
                stock: stock
                    ? { ...stock, symbol: safeSymbol, company: stock.company ?? '', type: stock.type ?? '' }
                    : null,
                initialPrices,
                symbol: safeSymbol,
            },
        ...
        console.error(error);
        return { props: { stock: null, initialPrices: [], symbol: symbol ?? null } };
    }
};
